...
// to get any random word from the prgoramming list of game.js
function randomWord() {
    fullAudio.play();
    count = 0
    wrongGuess = []
    ...
    startTimer = setInterval(function () {
        ...
        if (time < 2) {
            ...
            gameover.onended = function () {
                location.href = "./gameover.html?page=programming"
                randomWord();
            }
        }
    }, 1000);

    let randomSet = allProgrammingWords[Math.floor(Math.random() * allProgrammingWords.length)];
