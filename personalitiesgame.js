...
// to get any random word from the personalities list of game.js
function randomWord() {
    fullAudio.play();
    count = 0
    wrongGuess = []
    ...
    startTimer = setInterval(function () {
        ...
        if (time < 2) {
            ...
            gameover.onended = function () {
                location.href = "./gameover.html?page=personality"
                randomWord();
            }
        }
    }, 1000);

    let randomSet = allPersonalitiesWords[Math.floor(Math.random() * allPersonalitiesWords.length)];
